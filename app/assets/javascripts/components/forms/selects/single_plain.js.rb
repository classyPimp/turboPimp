module Forms
  module Selects
    class SinglePlain < RW
      expose
      #PROPS
      #server_feed: {url: #{url}}

      def component_did_mount
        @model_ats = props.model.attributes
      ...
      def prepare_options(options)
        ...
        options.each do |option|
          if @model_ats[props.attr] == option.value
      ...
      def select(option)
        ...
        current_selected = state.selected.delete_at(0)
        state.selected << selected
        state.options << current_selected if current_selected
      ...
      def collect
        x = state.selected[0]
        if x
          props.model.attributes[props.attr] = x.value
        else
          props.model.attributes[props.attr] = x
        end 
      end
